...
export default function UniversityFilter() {
  ...
  return (
    <div className="space-y-3">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className={university ? "text-brand-navy font-medium" : "text-gray-400"}>
              {university ? university.name : "เลือกมหาวิทยาลัย"}
            </span>
          </div>
          <div className="flex items-center gap-2">
            {selectedUni && (
          ...
          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-20 overflow-hidden">
            {UNIVERSITIES.map((uni) => (
              ...
              >
                ...
                <p className="text-xs text-gray-400 mt-0.5">{uni.zones.length} โซน</p>
              </button>
            ))}
          </div>
        )}
      ...
      {/* Zone selector — shows only when university is selected */}
      {university && (
