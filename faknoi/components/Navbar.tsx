...
export default function Navbar({ username }: { username: string }) {
  ...
  return (
    <>
      {/* Desktop top navbar */}
      <header className="hidden md:block bg-white/80 backdrop-blur-md border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          <Link href="/dashboard" className="flex items-center gap-2">
            ...
            <span className="font-extrabold text-brand-navy tracking-tight">FakNoi</span>
          </Link>
                ...
                <Link key={href} href={href}
                  className={clsx(
                    ...
                    active
                      ...
                      : "text-gray-500 hover:text-brand-navy hover:bg-gray-100"
                  )}>
      ...
      {/* Mobile top bar */}
      <header className="md:hidden bg-white/80 backdrop-blur-md border-b border-gray-100 sticky top-0 z-50"
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-t border-gray-100"
          ...
          {navItems.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link key={href} href={href}
                ...
                <span className={clsx("text-[10px] font-medium", active && "font-bold")}>{label}</span>
              </Link>
            );
          })}
        </div>
